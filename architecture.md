...
### AuthCraft.Cimd.DuendeIdentityServer (Integration Library)
...
**Key Components:**
- `ClientIdMetadataDocumentClientStore`: a `IClientStore` implementation that uses the HTTP client from `AuthCraft.Cimd` to fetch the OAuth client metadata document, validate it and map it to the Duende Identity Server client model
...
- `AuthCraft.Cimd` library to be reused across different authorization server frameworks
- `AuthCraft.DuendeIdentityServer` library to provide Duende Identity Server specific integration
- `AuthCraft.{OAuthServerName}` future libraries to provide integration with more OAuth server implementations in .NET 
...
The integration library (`AuthCraft.Cimd.DuendeIdentityServer`) provides:
...
### AuthCraft.Cimd.DuendeIdentityServer
- AuthCraft.Cimd (core library)
...
- Requires AuthCraft.Cimd package
...
```
Install-Package AuthCraft.Cimd.DuendeIdentityServer
```
(This automatically includes AuthCraft.Cimd as a dependency)

For custom integrations:
```
Install-Package AuthCraft.Cimd
```
...
1. Referencing the core `AuthCraft.Cimd` package
